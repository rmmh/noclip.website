...
export interface MPHbin {
    ...
    nodes: MPHNode[];
    matrixCount: number;
    matrixNodeIndices: Uint32Array;
    matrixBlendCounts: Uint32Array;
    scaleFactor: number;
}
...
export function parseMPH_Model(buffer: ArrayBufferSlice, sharedTexture: MPHTexture | null = null): MPHbin {
    ...
    let matrixNodeIndices = new Uint32Array(0);
    let matrixBlendCounts = new Uint32Array(0);
    if (hasMatrixNodeIndices) {
        matrixNodeIndices = buffer.createTypedArray(Uint32Array, matrixNodeIndexOffset, matrixCount);
        matrixBlendCounts = hasMatrixBlendCounts ?
            buffer.createTypedArray(Uint32Array, matrixBlendCountOffset, matrixCount) : new Uint32Array(matrixCount);
    }
